...
#define BOOST_TEST_MODULE IoTSiteWiseTest
#ifdef _WIN32
#include <Windows.h>
#endif
...
using iotsitewise_test::GetOdbcErrorMessage;
...
BOOST_AUTO_TEST_CASE(TestDriverConnectionUsingDupCredStringWithWrongUidPwd) {
  ...
  ExpectConnectionReject(
      ...
      "Failed to establish connection to IoT SiteWise.\nUnrecognizedClient"
...
BOOST_AUTO_TEST_CASE(TestDriverConnectionUsingDupCredStringWithWrongPwd) {
  ...
  ExpectConnectionReject(
      connectionString, "08001",
      "Failed to establish connection to IoT SiteWise.\nInvalidSignature"
      "Exception: The request signature we calculated does not match the "
...
BOOST_AUTO_TEST_CASE(TestSQLConnectionUsingOktaUidPwd) {
  if (CheckEnvVarSetToTrue("ENABLE_OKTA_TEST")) {
    ...
    std::string  connectionString =
      ...
      "logOutput=" + GetEnv("IOTSITEWISE_LOG_PATH", "") + ";"
      "logLevel=" + GetEnv("IOTSITEWISE_LOG_LEVEL", "2") + ";";
