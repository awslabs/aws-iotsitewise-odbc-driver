...
BOOST_AUTO_TEST_CASE(TestSQLDescribeParam) {
  ...
  std::vector< SQLWCHAR > request = MakeSqlBuffer("SELECT table_name FROM system.tables");
...
BOOST_AUTO_TEST_CASE(TestSetGetCursorName) {
  ...
  BOOST_CHECK_EQUAL(
      iotsitewise::odbc::utility::SqlWcharToString(cursorName.data()),
      iotsitewise::odbc::utility::SqlWcharToString(cursorNameRes));
...
BOOST_AUTO_TEST_CASE(TestSQLSetCursorNameTruncated) {
  ...
  BOOST_CHECK_EQUAL(iotsitewise::odbc::utility::SqlWcharToString(cursorNameRes),
                    "curso");
...
BOOST_AUTO_TEST_CASE(TestSQLSetCursorNameMultipleTimes) {
  ...
  BOOST_CHECK_EQUAL(
      iotsitewise::odbc::utility::SqlWcharToString(cursorName2.data()),
      iotsitewise::odbc::utility::SqlWcharToString(cursorNameRes));
  BOOST_CHECK_EQUAL(resLen, 7);
}
...
BOOST_AUTO_TEST_CASE(TestSQLGetFunctions) {
  ...
  for (int i = 0; i < 58; i++) {
    ...
    if (retcode != SQL_SUCCESS) {
      BOOST_FAIL("Failed for " + iotsitewise::odbc::common::LongToString(i)
