...
/**
 * Connect to IoT SiteWise
 */
void OdbcTestSuite::ConnectToSW(int32_t odbcVer) {
...
void OdbcTestSuite::CheckSQLDiagnosticError(
  ...
  const std::string sqlState =
      iotsitewise::odbc::utility::SqlWcharToString(state);
...
void OdbcTestSuite::CreateGenericDsnConnectionString(
    ...
    const std::string& uid, const std::string& pwd, const bool includeSWCred,
    const std::string& SWUsername, const std::string& SWPassword,
  ...
  std::string logLevel = GetEnv("IOTSITEWISE_LOG_LEVEL", "2");
  std::string region = GetEnv("AWS_REGION", "us-east-1");
  ...
  if (includeSWCred) {
    // Append the IoT SiteWise credentials
    std::string swAuthentication = "";

    switch (testAuthType) {
      case AuthType::Type::IAM:
        swAuthentication = 
            "accessKeyId=" + SWUsername + ";"
            "secretKey=" + SWPassword + ";";
      default:
        break;
    }

    if (!swAuthentication.empty()) {
      connectionString.append(swAuthentication);
    }
  }
...
void OdbcTestSuite::CreateDsnConnectionStringForAWS(
  ...
  connectionString =
            ...
            "region=" + region + ";"
            "endpointOverride=iotsitewise.us-east-1.amazonaws.com;"
            "logOutput=" + logPath + ";"
            "logLevel=" + logLevel + ";";
