...
struct SqlGetInfoTestSuiteFixture : odbc::OdbcTestSuite {
  ...
  /**
   * Connect to the IoT SiteWise server with the database name
   *
   * @param databaseName Database Name
   */
  void connectToSWServer() {
...
BOOST_AUTO_TEST_CASE(TestValues) {
...
#if defined(WIN32)
  CheckStrInfo(SQL_DRIVER_NAME, "iotsitewise.odbc.dll");
#elif defined(__APPLE__)
  CheckStrInfo(SQL_DRIVER_NAME, "libiotsitewise-odbc.dylib");
#elif defined(__linux__)
  CheckStrInfo(SQL_DRIVER_NAME, "libiotsitewise-odbc.so");
#endif
  CheckStrInfo(SQL_DBMS_NAME, "AWS IoT SiteWise");
...
#else
  CheckStrInfo(SQL_DATA_SOURCE_NAME, "IoT SiteWise DSN");
#endif
  ...
  CheckStrInfo(SQL_SERVER_NAME, "AWS IoT SiteWise");
