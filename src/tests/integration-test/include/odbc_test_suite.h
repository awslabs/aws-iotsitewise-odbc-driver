...
struct OdbcTestSuite {
  ...
  /**
   * Connect to IoT SiteWise (ODBC Ver is 3 by default)
   */
  void ConnectToSW(int odbcVer = SQL_OV_ODBC3);
   ...
   * { accessKeyId/secretKey, idPUsername, idPPassword } if @param includeSWCred
   * is set to true.
   */
  void CreateGenericDsnConnectionString(
      std::string& connectionString, AuthType::Type testAuthType,
      const std::string& uid, const std::string& pwd,
      bool includeSWCred = false, const std::string& SWUsername = std::string(),
      const std::string& SWPassword = std::string(),
