...
#include "iotsitewise/odbc/diagnostic/diagnostic_record_storage.h"

using namespace iotsitewise::odbc;
using namespace iotsitewise::odbc::config;
using iotsitewise::odbc::common::EncodeURIComponent;
...
void CheckConnectionConfig(const Configuration& cfg) {
  ...
  BOOST_CHECK_EQUAL(iotsitewise::odbc::common::ToLower(cfg.ToConnectString()),
                    iotsitewise::odbc::common::ToLower(expectedStr));
}
...
BOOST_AUTO_TEST_CASE(TestParseDriverVersion) {
  using iotsitewise::odbc::system::ui::DsnConfigurationWindow;
