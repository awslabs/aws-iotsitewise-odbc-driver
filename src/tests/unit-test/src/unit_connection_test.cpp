...
#define BOOST_TEST_MODULE IoTSiteWiseUnitTest
#include <string>
...
struct ConnectionUnitTestSuiteFixture : OdbcUnitTestSuite {
  ...
  void getLogOptions(Configuration& config) const {
    ...
    std::string logLevelStr = GetEnv("IOTSITEWISE_LOG_LEVEL", "2");
  ...
  void CheckConnectError(Configuration& cfg, const std::string& expectedMsg) {
    ...
    std::ostream* original =
        iotsitewise::odbc::Logger::GetLoggerInstance()->GetLogStream();
    iotsitewise::odbc::Logger::GetLoggerInstance()->SetLogStream(
        static_cast< std::ostream* >(&ss));

    dbc->Establish(cfg);

    iotsitewise::odbc::Logger::GetLoggerInstance()->SetLogStream(original);
...
BOOST_AUTO_TEST_CASE(TestEstablishUsingKey) {
  ...
  cfg.SetAccessKeyId("AwsSWUnitTestKeyId");
  cfg.SetSecretKey("AwsSWUnitTestSecretKey");
  getLogOptions(cfg);
...
BOOST_AUTO_TEST_CASE(TestEstablishUsingAAD) {
  ...
  BOOST_CHECK(iotsitewise::odbc::utility::SqlWcharToString(userName)
              == "aad_valid_user");
  BOOST_CHECK(IsSuccessful());
}
