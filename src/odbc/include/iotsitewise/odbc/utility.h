...
#ifndef _IOTSITEWISE_ODBC_UTILITY
#define _IOTSITEWISE_ODBC_UTILITY
...
using iotsitewise::odbc::common::IntoLower;
...
/**
 * Checks if an environment variable is not set to false. Case insensitive.
 * @param envVar Environment variable name
 *
 * @return true if environment variable is not set to "FALSE", true otherwise
 */
IGNITE_IMPORT_EXPORT bool CheckEnvVarNotSetToFalse(const std::string& envVar);
...
inline std::ostream& operator<<(std::ostream& out, const std::wstring& value) {
  out << iotsitewise::odbc::utility::ToUtf8(value);
  return out;
}
}  // namespace std

#endif  //_IOTSITEWISE_ODBC_UTILITY
