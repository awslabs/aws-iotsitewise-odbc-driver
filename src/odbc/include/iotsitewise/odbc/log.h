...
#ifndef _IOTSITEWISE_ODBC_LOG
#define _IOTSITEWISE_ODBC_LOG
...
#define DEFAULT_LOG_PATH iotsitewise::odbc::Logger::GetDefaultLogPath()
...
#define WRITE_MSG_TO_STREAM(param, logLevel, logStream)                       \
  {                                                                           \
    std::shared_ptr< iotsitewise::odbc::Logger > p =                           \
        iotsitewise::odbc::Logger::GetLoggerInstance();                        \
    if (p->GetLogLevel() >= logLevel && (p->IsEnabled() || p->EnableLog())) { \
      ...
      std::unique_ptr< iotsitewise::odbc::LogStream > lstream(                 \
          new iotsitewise::odbc::LogStream(p.get()));                          \
      std::string msg_prefix;                                                 \
      switch (logLevel) {                                                     \
        case iotsitewise::odbc::LogLevel::Type::DEBUG_LEVEL:                   \
          msg_prefix = "DEBUG MSG: ";                                         \
          break;                                                              \
        case iotsitewise::odbc::LogLevel::Type::INFO_LEVEL:                    \
          msg_prefix = "INFO MSG: ";                                          \
          break;                                                              \
        case iotsitewise::odbc::LogLevel::Type::WARNING_LEVEL:                 \
          msg_prefix = "WARNING MSG: ";                                       \
          break;                                                              \
        case iotsitewise::odbc::LogLevel::Type::ERROR_LEVEL:                   \
      ...
      *lstream << "TID: " << std::this_thread::get_id() << " " << tStr        \
               << msg_prefix << " "                                           \
               << iotsitewise::odbc::Logger::GetBaseFileName(__FILE__) << ":"  \
...
#define LOG_DEBUG_MSG(param) \
  WRITE_LOG_MSG(param, iotsitewise::odbc::LogLevel::Type::DEBUG_LEVEL)

#define LOG_DEBUG_MSG_TO_STREAM(param, logStream)                           \
  WRITE_MSG_TO_STREAM(param, iotsitewise::odbc::LogLevel::Type::DEBUG_LEVEL, \
                      logStream)

// Info messages are messages that document the application flow
#define LOG_INFO_MSG(param) \
  WRITE_LOG_MSG(param, iotsitewise::odbc::LogLevel::Type::INFO_LEVEL)

#define LOG_INFO_MSG_TO_STREAM(param, logStream)                           \
  WRITE_MSG_TO_STREAM(param, iotsitewise::odbc::LogLevel::Type::INFO_LEVEL, \
                      logStream)

// Warning messages display warnings
#define LOG_WARNING_MSG(param) \
  WRITE_LOG_MSG(param, iotsitewise::odbc::LogLevel::Type::WARNING_LEVEL)

#define LOG_WARNING_MSG_TO_STREAM(param, logStream)                           \
  WRITE_MSG_TO_STREAM(param, iotsitewise::odbc::LogLevel::Type::WARNING_LEVEL, \
                      logStream)

// Error messages display errors
#define LOG_ERROR_MSG(param) \
  WRITE_LOG_MSG(param, iotsitewise::odbc::LogLevel::Type::ERROR_LEVEL)

#define LOG_ERROR_MSG_TO_STREAM(param, logStream)                           \
  WRITE_MSG_TO_STREAM(param, iotsitewise::odbc::LogLevel::Type::ERROR_LEVEL, \
                      logStream)
...
class Logger {
 ...
 private:
   ...
   * Log file format: iotsitewise_odbc_YYYYMMDD.log
   */
