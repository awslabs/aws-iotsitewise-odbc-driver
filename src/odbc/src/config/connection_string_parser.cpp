...
void ConnectionStringParser::ParseConnectionString(
  ...
  while (!connect_str.empty()) {
    ...
    size_t attr_eq_pos = connect_str.substr(attr_begin).find('=') + attr_begin;
    ...
    if (attr_begin < attr_eq_pos) {
      ...
      std::string key = iotsitewise::odbc::utility::Trim(
          connect_str.substr(attr_begin, attr_eq_pos - attr_begin));
      std::string value = iotsitewise::odbc::utility::Trim(connect_str.substr(
...
void ConnectionStringParser::HandleAttributePair(
  ...
  std::string lKey = iotsitewise::odbc::common::ToLower(key);
  ...
  if (lKey == Key::dsn) {
    ...
    if (!iotsitewise::odbc::common::AllDigits(value)) {
      if (diag) {
        diag->AddStatusRecord(
            SqlState::S01S02_OPTION_VALUE_CHANGED,
    ...
    std::string val = utility::Trim(iotsitewise::odbc::common::ToLower(value));
...
ConnectionStringParser::StringToBool(const std::string& value) {
  std::string lower = iotsitewise::odbc::common::ToLower(value);
