...
TypeInfoQuery::TypeInfoQuery(diagnostic::DiagnosableAdapter& diag,
                             int16_t sqlType)
    : iotsitewise::odbc::query::Query(
        diag, iotsitewise::odbc::query::QueryType::TYPE_INFO),
      columnsMeta(),
      executed(false),
      fetched(false),
      types(),
  ...
  columnsMeta.push_back(ColumnMeta(sch, tbl, "SEARCHABLE", ScalarType::INT,
                                   Nullability::NO_NULL));
  ...
  if (sqlType == SQL_ALL_TYPES) {
    types.push_back(static_cast< int8_t >(ScalarType::STRING));
    ...
    types.push_back(static_cast< int8_t >(ScalarType::INT));
...
SqlResult::Type TypeInfoQuery::GetColumn(uint16_t columnIdx,
  ...
  switch (columnIdx) {
    ...
    case ResultColumn::LITERAL_PREFIX: {
      if (currentType == static_cast< int8_t >(ScalarType::STRING)) {
        buffer.PutString("'");
      } else {
        buffer.PutNull();
      }
