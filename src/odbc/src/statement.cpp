...
#include "iotsitewise/odbc/query/column_privileges_query.h"
#include "iotsitewise/odbc/query/data_query.h"
#include "iotsitewise/odbc/query/foreign_keys_query.h"
#include "iotsitewise/odbc/query/primary_keys_query.h"
#include "iotsitewise/odbc/query/procedure_columns_query.h"
#include "iotsitewise/odbc/query/procedures_query.h"
#include "iotsitewise/odbc/query/statistics_query.h"
#include "iotsitewise/odbc/query/special_columns_query.h"
#include "iotsitewise/odbc/query/table_metadata_query.h"
#include "iotsitewise/odbc/query/table_privileges_query.h"
#include "iotsitewise/odbc/query/type_info_query.h"
...
SqlResult::Type Statement::InternalSetAttribute(int attr, void* value,
                                                SQLINTEGER) {
  LOG_DEBUG_MSG("InternalSetAttribute is called with attr " << attr << " (0x" << std::hex << attr << std::dec << ")");
  switch (attr) {
    ...
    case SQL_ATTR_RETRIEVE_DATA: {
      ...
      LOG_DEBUG_MSG("SQL_ATTR_RETRIEVE_DATA set to: " << retrieveData 
                    << " (SQL_RD_ON=" << SQL_RD_ON << ")");
      ...
      // Store the setting but don't enforce it strictly
      // The driver will always retrieve data when fetching
      break;
    }
    ...
    default: {
      LOG_DEBUG_MSG("InternalSetAttribute: Unsupported attribute " << attr << " (0x" << std::hex << attr << std::dec << ")");
      AddStatusRecord(SqlState::SHYC00_OPTIONAL_FEATURE_NOT_IMPLEMENTED,
...
SqlResult::Type Statement::InternalGetAttribute(int attr, void* buf, SQLINTEGER,
  ...
  switch (attr) {
    ...
    default: {
      LOG_DEBUG_MSG("InternalGetAttribute: Unsupported attribute " << attr << " (0x" << std::hex << attr << std::dec << ")");
      AddStatusRecord(SqlState::SHYC00_OPTIONAL_FEATURE_NOT_IMPLEMENTED,
                      "Specified attribute is not supported.");
...
SqlResult::Type Statement::InternalGetColumnAttribute(
  ...
  if (colIdx > meta->size() || colIdx < 1) {
    AddStatusRecord(SqlState::SHY000_GENERAL_ERROR,
                    ...
                    iotsitewise::odbc::LogLevel::Type::ERROR_LEVEL, 0, colIdx);
...
SqlResult::Type Statement::InternalGetCursorName(SQLWCHAR* nameBuf,
  ...
  // '\0'
  size_t resultLen = iotsitewise::odbc::utility::CopyUtf8StringToSqlWcharString(
...
SqlResult::Type Statement::InternalSetCursorName(SQLWCHAR* name,
  ...
  std::string cursorName =
      iotsitewise::odbc::utility::SqlWcharToString(name, nameLen);
