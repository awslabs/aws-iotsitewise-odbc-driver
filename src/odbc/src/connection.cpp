...
#include "iotsitewise/odbc/dsn_config.h"
#include "iotsitewise/odbc/environment.h"
#include "iotsitewise/odbc/log.h"
#include "iotsitewise/odbc/statement.h"
#include "iotsitewise/odbc/system/system_dsn.h"
#include "iotsitewise/odbc/utility.h"

#include "iotsitewise/odbc/authentication/aad.h"
#include "iotsitewise/odbc/authentication/okta.h"
  ...
  if (!awsSDKReady_) {
    ...
    if (!awsSDKReady_) {
      Aws::Utils::Logging::LogLevel awsLogLvl = GetAWSLogLevelFromString(
          ignite::odbc::common::GetEnv("SW_AWS_LOG_LEVEL"));
...
SqlResult::Type Connection::InternalGetInfo(
  ...
  if (res != SqlResult::AI_SUCCESS) {
    ...
    AddStatusRecord(SqlState::SHYC00_OPTIONAL_FEATURE_NOT_IMPLEMENTED, ss.str(),
                    iotsitewise::odbc::LogLevel::Type::INFO_LEVEL);
  }
...
SqlResult::Type Connection::InternalEstablish(
  ...
  if (!connected) {
    std::string errMessage = "Failed to establish connection to IoT SiteWise.\n";
...
std::shared_ptr< Aws::IoTSiteWise::IoTSiteWiseClient >
...
SqlResult::Type Connection::InternalGetAttribute(int attr, void* buf,
  ...
  switch (attr) {
    case SQL_ATTR_CONNECTION_DEAD: {
      ...
      *val = client_ ? SQL_CD_FALSE : SQL_CD_TRUE;
...
void Connection::SetClientProxy(Aws::Client::ClientConfiguration& clientCfg) {
  ...
  // proxy host
  std::string proxyHost = utility::Trim(GetEnv("SW_PROXY_HOST"));
  if (!proxyHost.empty()) {
  ...
  // proxy port
  int proxyPort = 0;
  std::string portStr = utility::Trim(GetEnv("SW_PROXY_PORT"));
  if (!portStr.empty()) {
  ...
  // proxy scheme
  std::string proxyScheme = utility::Trim(GetEnv("SW_PROXY_SCHEME"));
  ...
  // proxy user name
  std::string proxyUser = utility::Trim(GetEnv("SW_PROXY_USER"));
  if (!proxyUser.empty()) {
  ...
  // proxy user password
  std::string proxyPassword = utility::Trim(GetEnv("SW_PROXY_PASSWORD"));
  ...
  std::string proxySSLCertPath =
      utility::Trim(GetEnv("SW_PROXY_SSL_CERT_PATH"));
  ...
  std::string proxySSLCertType =
      utility::Trim(GetEnv("SW_PROXY_SSL_CERT_TYPE"));
  ...
  // proxy SSL key path
  std::string proxySSLKeyPath = utility::Trim(GetEnv("SW_PROXY_SSL_KEY_PATH"));
  ...
  // proxy SSL key type
  std::string proxySSLKeyType = utility::Trim(GetEnv("SW_PROXY_SSL_KEY_TYPE"));
  ...
  std::string proxySSLKeyPassword =
      utility::Trim(GetEnv("SW_PROXY_SSL_KEY_PASSWORD"));
...
bool Connection::TryRestoreConnection(const config::Configuration& cfg,
  ...
  if (authType == AuthType::Type::OKTA) {
    ...
    samlCredProvider_ =
        std::make_shared< iotsitewise::odbc::IoTSiteWiseOktaCredentialsProvider >(
    ...
    std::shared_ptr< Aws::STS::STSClient > stsClient = GetStsClient();
    samlCredProvider_ =
        std::make_shared< iotsitewise::odbc::IoTSiteWiseAADCredentialsProvider >(
            cfg, httpClient, stsClient);
    samlCredProvider_->GetAWSCredentials(credentials, errInfo);
    ...
    err = IgniteError(IgniteError::IGNITE_ERR_SW_CONNECT, errMsg.data());
  ...
  if (credentials.IsExpiredOrEmpty()) {
    ...
    err = IgniteError(IgniteError::IGNITE_ERR_SW_CONNECT, errInfo.data());
...
#endif
  // pass driver info to IoT SiteWise as user agent
  clientCfg.userAgent = "sw-odbc." + utility::GetFormatedDriverVersion() + " on " + platform;
  ...
  client_ = CreateIoTSiteWiseClient(credentials, clientCfg);

  const std::string& endpoint = cfg.GetEndpoint();
  // endpoint could not be set to empty string
  if (!endpoint.empty()) {
    client_->OverrideEndpoint(endpoint);
    LOG_DEBUG_MSG("endpoint is set to " << endpoint);
  }
  
  // try a simple query with client
  Aws::IoTSiteWise::Model::ExecuteQueryRequest queryRequest;
  queryRequest.SetQueryStatement("SELECT table_name FROM system.tables");
 
  Aws::IoTSiteWise::Model::ExecuteQueryOutcome outcome =
      client_->ExecuteQuery(queryRequest);
  if (!outcome.IsSuccess()) {
    auto error = outcome.GetError();
    LOG_DEBUG_MSG("ERROR: " << error.GetExceptionName() << ": "
                            << error.GetMessage());
 
    err = IgniteError(IgniteError::IGNITE_ERR_SW_CONNECT,
...
std::shared_ptr< Aws::IoTSiteWise::IoTSiteWiseClient >
Connection::CreateIoTSiteWiseClient(
    const Aws::Auth::AWSCredentials& credentials,
    const Aws::Client::ClientConfiguration& clientCfg) {
  return std::make_shared< Aws::IoTSiteWise::IoTSiteWiseClient >(
