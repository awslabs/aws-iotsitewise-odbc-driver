...
#include "iotsitewise/odbc/system/ui/dsn_configuration_window.h"
...
DsnConfigurationWindow::DsnConfigurationWindow(Window* parent,
    ...
    : CustomWindow(parent, L"IoTSiteWiseConfigureDsn",
                   L"Configure AWS IoT SiteWise DSN"),
      width(450),
      height(425),
      nameEdit(),
      nameLabel(),
...
int DsnConfigurationWindow::CreateLogSettingsGroup(int posX, int posY,
  ...
  logPathLabel = CreateLabel(
      ...
      L"Log Path:\n(the log file name format is iotsitewise_odbc_YYYYMMDD.log)",
      ...
      switch (LOWORD(wParam)) {
        ...
        case ChildId::MAX_CONNECTIONS_EDIT: {
          if (created) {
            ...
            int16_t maxCon =
                iotsitewise::odbc::common::LexicalCast< int16_t >(maxConStr);
...
void DsnConfigurationWindow::RetrieveConnectionParameters(
  ...
  int32_t connectionTimeout =
      connectionTimeoutStr.empty()
          ? 0
          : iotsitewise::odbc::common::LexicalCast< int32_t >(
  ...
  int32_t reqTimeout =
      reqTimeoutStr.empty()
          ? 0
          : iotsitewise::odbc::common::LexicalCast< int32_t >(reqTimeoutStr);
  if (reqTimeout < 0) {
  ...
  int32_t maxRetryCountClient =
      maxRetryCountStr.empty()
          ? 0
          : iotsitewise::odbc::common::LexicalCast< int32_t >(maxRetryCountStr);
  ...
  int32_t maxCon =
      maxConStr.empty()
          ? 0
          : iotsitewise::odbc::common::LexicalCast< int32_t >(maxConStr);
