...
using iotsitewise::odbc::utility::FromUtf8;

#ifdef _WIN32
bool DisplayConnectionWindow(void* windowParent, Configuration& config) {
  using namespace iotsitewise::odbc::system::ui;
  ...
  } catch (const iotsitewise::odbc::IgniteError& err) {
...
bool InternalRegisterDsn(const Configuration& config, LPCSTR driver) {
  ...
  try {
    ...
    return true;
  } catch (iotsitewise::odbc::IgniteError& err) {
