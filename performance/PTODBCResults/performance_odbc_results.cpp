...
auto RecordBindingFetching = [](SQLHSTMT& hstmt,
  ...
  for (size_t iter = 0; iter < ITERATION_COUNT; iter++) {
    ...
    // Performance tests use WCHAR exclusively since the AWS IoT SiteWise ODBC
...
int main(int argc, char** argv) {
  ...
  std::string defaultConnStr = "DSN=iotsitewise-iam;";
      ...
      if (strcmp(argv[i], "--large-test") == 0) {
        ...
        std::cout
            ...
            << "Valid arguments are:\n"
               ...
               "key must also be provided. Defaults to us-east-1.\n"
  ...
  if (!accessKeyId.empty() && !secretKey.empty()) {
    specialConnStr = std::string("Driver=AWS IoT SiteWise ODBC Driver;")
